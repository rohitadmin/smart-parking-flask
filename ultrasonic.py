...
if __name__ == '__main__':
    try:
        while True:
            ...
            camera = PiCamera()
            if dist_measured < 20:
                print ("Vehicle Detected at Distance:",dist_measured, "Time Taken",timeD)
                camera.start_preview()
                time.sleep(3)
                camera.capture('/home/pi/project/flasksmartparking/vehicle_image/image0.jpg')
                final_url= url+"&field1=%s" %(dist_measured)
                s= urllib.request.urlopen(final_url)
                print("Vehicle Image captured")
                camera.stop_preview()
            else :
                print ("No Vehicle at Distance",dist_measured, "Time Taken",timeD)
